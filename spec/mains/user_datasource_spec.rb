...
RSpec.describe UserDatasource, "Local Registration and Account Storage. " do
  ...
  context "Core Operations" do
    ...
    context "Credentials" do
      it "Add" do
        ...
        expect(service.authenticate!("someuser","someuser pwd")).to be_a SknHash
      end
      it "Save" do
        ...
        expect(service.authenticate!("newuser","newuser pwd")).to be_a SknHash
      end
      it "Delete" do
        crd = creds[:emowner].to_hash
        expect(service.send(:credentials_delete,crd)).to be true
        expect(service.authenticate!(crd[:username],crd[:password])).to be nil
      end
      it "Restore" do
    ...
    context "Accounts" do
      it "Add" do
        ...
        expect(service.account_for("someuser")&.balance).to eq 1200
      end
      it "Save" do
        crd = {username: "someuser",
               balance: 1200
      ...
      it "Delete" do
        ...
        expect(service.account_for(crd[:username])&.balance).to eq 0
      end
      it "Restore" do
  ...
  context "Application UseCases" do
    ...
    it "Register User. " do
      expect(service.register("newuser", "newuser pwd", ["view_only"])).to be true
    end
    it "Unregister User. " do
      crd = SknHash.new(creds[:emowner].to_hash)
      expect(service.unregister(crd.username)).to be true
    end
    it "Account for User. " do
      crd = SknHash.new(accts[:emowner].to_hash)
      expect(service.account_for(crd.username)).to be_a SknHash
    end
    it "Update Account. " do
      crd = SknHash.new(accts[:emowner].to_hash)
      expect(service.account_update_for(crd.username, 1000)).to be true
    end
    it "Authenticate Credentials. " do
      crd = SknHash.new(creds[:emuser].to_hash)
      expect(service.authenticate!(crd.username, crd.password)).to be_a SknHash
      expect(service.authenticate!(crd.username, "Suzy")).to be_nil
    end
  end
end
